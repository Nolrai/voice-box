...
getDollHearing ::
  ...
  Vector Double ->
  Maybe EarResult
getDollHearing dollEar sampleRate samples = do
  pulses <- unzipMapOfVectors $ runReader (processDollEar samples) (sampleRate, dollEar)
...
tapPulses :: V.Vector Double -> EarR (Map Int (Vector Int8))
...
linearPhaseIntensity deltaPhi = min 1.0 (wrapPhase deltaPhi / pi)
